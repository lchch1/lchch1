...
class SoundSetting2Activity : AppCompatActivity() {
    ...
    // [P1 Fix] onPause, onDestroy 추가 - 대중교통 모드 설정 저장
    ...
    private fun saveSwitchStates() {
        val soundPref = getSharedPreferences("transit2_settings", MODE_PRIVATE)
        with(soundPref.edit()) {
        ...
        val dogPref = getSharedPreferences("dog2_settings", MODE_PRIVATE)
        dogPref.edit().putBoolean("Ssw6_state", switchStates[R.id.Ssw6] ?: true).apply()

        val kidPref = getSharedPreferences("kid2_settings", MODE_PRIVATE)
        kidPref.edit().putBoolean("Ssw5_state", switchStates[R.id.Ssw5] ?: true).apply()
    }
    ...
    private fun loadSwitchStates() {
        ...
        val dogPref = getSharedPreferences("dog2_settings", MODE_PRIVATE)
        switchStates[R.id.Ssw6] = dogPref.getBoolean("Ssw6_state", true)
